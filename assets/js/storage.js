...
async function updateProductInDB(id, productData) {
    ...
    try {
        // نحدث created_at لكي يظهر الاسكراب في الأعلى عند الترتيب
...
function addToCart(product, quantity = 1, selectedSize, selectedColor) {
    ...
    if (typeof showToast === 'function') {
        showToast('تمت إضافة الاسكراب للسلة بنجاح');
    } else {
        alert('تمت إضافة الاسكراب للسلة');
    }
}
...
function removeFromCart(cartItemId) {
    ...
    if (typeof showToast === 'function') showToast('تم حذف الاسكراب من السلة');
}

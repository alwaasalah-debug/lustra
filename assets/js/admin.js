...
async function handleEdit(id) {
    try {
        ...
        if (!product) {
            alert('لم يتم العثور على الاسكراب');
            return;
        }

        // Set form title
        const formTitle = document.querySelector('.form-panel h2');
        formTitle.textContent = 'تعديل الاسكراب';
        ...
        submitBtn.textContent = 'تحديث الاسكراب';
        ...
        alert('حدث خطأ أثناء تحميل بيانات الاسكراب');
    }
}
...
function resetFormToAddMode() {
    ...
    document.querySelector('.form-panel h2').textContent = 'إضافة سكراب جديد';
    document.querySelector('#add-product-form button[type="submit"]').textContent = 'حفظ الاسكراب';
...
async function handleAddProduct(e) {
    ...
    try {
        ...
        if (id) {
            ...
            alert('تم تحديث الاسكراب بنجاح!');
            resetFormToAddMode();
            renderProductsTable();
        } else {
            // Create Mode
            if (imageUrls.length === 0) {
                alert('يرجى اختيار صورة للاسكراب');
...
async function renderProductsTable() {
    ...
    if (products.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;">لا توجد اسكرابات</td></tr>';
        return;
    }

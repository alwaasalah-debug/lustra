...
function removeItem(itemId) {
  if (confirm('هل أنت متأكد من حذف هذا الاسكراب؟')) {
...
async function checkoutCOD() {
  ...
  try {
    ...
    // ------ إرسال إشعار عبر سيرفر Vercel بأمان ------
    try {
      let itemsText = orderData.items.map(item => `- ${item.name} (${item.size}) | الكمية: ${item.quantity} | السعر: ${item.price} ج.م`).join('\n');
...
💰 *الإجمالي:* ${orderData.total_price} ج.م
      `;

      // المناداة على الملف الآمن في السيرفر
      fetch('/api/telegram', {
        method: 'POST',
    ...
    // إعداد التحويل التلقائي لتطبيق واتس آب
    const whatsappMsg = `مرحباً،
لقد قمت بعمل طلب جديد من الموقع وأريد تأكيده:
👤 الاسم: ${orderData.customer_name}
مبلغ الأوردر: ${orderData.total_price} ج.م
📍 العنوان: ${orderData.governorate} - ${orderData.customer_address}

الرجاء مراجعة طلباتي في الموقع وتأكيد الشحن معي.`;

    // ⚠️ استبدل رقم الأصفار هنا برقم الواتس آب الخاص بالإدارة (يجب أن يبدأ بكود مصر 20 بدون علامة +) ⚠️
    const adminWhatsappNumber = "201016873077";
    const whatsappUrl = `https://wa.me/${adminWhatsappNumber}?text=${encodeURIComponent(whatsappMsg)}`;

    alert('تم استلام طلبك بنجاح! سيتم تحويلك الآن لتطبيق واتساب لتأكيد الطلب.');
